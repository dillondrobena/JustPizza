...
(function() {
  var addedPrice, adjustForDeletion, adjustTotal, calculateCheckBoxes, calculateTotal, customPrice, uncheckBoxes;
  ...
  $(document).ready(function() {
    ...
    $(':radio').change(function() {
      if ($('.custom_pizza').is(":checked")) {
        return $('#custom_pizza_fields').show();
      } else {
        uncheckBoxes($('#custom_pizza_fields'));
        return $('#custom_pizza_fields').hide();
      }
    });
    $(':checkbox').click(function() {
      return customPrice($(this));
    });
    $('.quantity').change(function() {
      return adjustTotal($(this));
    });
    $('.deletion').click(function() {
      return adjustForDeletion($(this));
    });
    $('body').on('cocoon:after-insert', function(e, insertedItem) {
      ...
      return $(insertedItem).find(':radio').change(function() {
        var pizzaDiv;
        pizzaDiv = $(this).hasClass('custom_pizza') ? $(this) : $(this).closest('.nested-fields').find('div:has(.custom_pizza)').find(':radio.custom_pizza');
        if ($(pizzaDiv).is(":checked")) {
          return $(pizzaDiv).parent().find('#custom_pizza_fields').show();
        } else {
          return $(pizzaDiv).parent().find('#custom_pizza_fields').hide();
        }
      });
    });
  });

  calculateTotal = function(arg) {
    var arrayIndex, localLabel, orderTotal, price, quantity;
    if (!$(arg).closest('#custom_pizza_fields').length > 0) {
      ...
      if (addedPrice[arrayIndex] === void 0) {
  ...
  adjustTotal = function(arg) {
    var arrayIndex, localLabel, orderTotal, price, radio;
    arrayIndex = $('.nested-fields').index($(arg).closest('.nested-fields'));
    orderTotal = $('#order_total');
    radio = $(arg).closest('.nested-fields').find(':radio:checked');
    localLabel = $(radio).siblings("label").text();
    if (radio.length !== void 0 && radio.length > 0) {
  ...
  adjustForDeletion = function(arg) {
    var arrayIndex, orderTotal;
    arrayIndex = $('.nested-fields').index($(arg).closest('.nested-fields'));
    orderTotal = $('#order_total');
    if (addedPrice[arrayIndex] !== void 0) {
      $(orderTotal).val((parseFloat($(orderTotal).val()) - addedPrice[arrayIndex]).toFixed(2));
      addedPrice[arrayIndex] = void 0;
    }
  };

  customPrice = function(arg) {
    var arrayIndex, label, orderTotal, quantity, toppingPrice;
